...
User.create = async (newUser) => {
  ...
  delete newUser.name;
  let insert = await query("INSERT INTO user SET ?", newUser);
  if (insert.insertId) {
    await query("INSERT INTO customer SET user_id = ?, name = ?", [
      insert.insertId,
      name,
    ]);
...
User.findBy = async (data, field, attach_additional_data = false) => {
  let row = await query(`SELECT * FROM user WHERE ${field} = ?`, data[field]);
...
User.login = async (value) => {
  let row = await query(`SELECT * FROM user WHERE mobile = ? OR email = ?`, [
    value,
    value,
  ]);
  if (row.length) {
...
User.getAll = async (start, limit, return_total, sort_data) => {
  ...
  str_query +=
    " ORDER BY " + sort_data.sort_by + " " + sort_data.sort_dir + " LIMIT ?, ?";

  if (return_total) {
    str_query += "; SELECT FOUND_ROWS() AS total_rows;";
  }

  let rows = await query(str_query, param_data);
...
User.getType = async (id) => {
  let row = await query(
    `SELECT ut.name FROM user u LEFT JOIN user_types ut ON u.user_type_id = ut.id WHERE u.id = ?`,
    id
  );
  if (row.length) {
    return row[0];
